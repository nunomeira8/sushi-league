...
export async function generateAwards(roomId: string) {
  const { data: existingAwards } = await supabase.from('room_awards').select('id').eq('room_id', roomId).limit(1);
  ...
  for (const category of categories) {
    ...
    const leaderboardWithoutRanks = categoryScores.map((score) => {
      ...
      const weightedScore = score.score * (CATEGORY_WEIGHTS[category] || 1);
    ...
    awardsToInsert.push({
      ...
      winner_player_name: hasWinner ? winners.map((player) => player.player_name).join(' / ') : '',
  ...
  awardsToInsert.push({
    ...
    winner_player_name: winners.map((player) => player.player_name).join(' / '),
    winner_score: winner.total_score,
    leaderboard_json: finalLeaderboard,
    is_final_winner: true,
  });

  await supabase.from('room_awards').upsert(awardsToInsert, {
